...
DEFAULT_IMAGE = "threefoldtech/js-ng:latest"
...
class ThreeBot(Container):
    ...
    def install(
        ...
        if os.path.exists(f"{PERSISTENT_STORE}/{name}"):
            Container.install(name, image, development)
        else:
            ...
            container = Container.install(name, image, development)
